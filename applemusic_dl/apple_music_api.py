...
class AppleMusicApi:
    ...
    def get_artist(
        self,
        artist_id: str,
        include: str = "albums", # Simplified include
        limit: int = 100,
        fetch_all: bool = True,
    ) -> dict:
        response = self.session.get(
    ...
    # --- NEW METHOD ---
    def get_syllable_lyrics(self, song_id: str) -> dict | None:
        """
        Fetches syllable-synced lyrics for a given song ID.
        Returns the raw TTML string if available, otherwise None.
        """
        response = self.session.get(
            f"{self.AMP_API_URL}/v1/catalog/{self.storefront}/songs/{song_id}/syllable-lyrics"
        )
        if response.status_code != 200:
            return None
        try:
            data = response.json()
            # The actual TTML is nested inside the response
