...
class DownloaderSong:
    ...
    def get_lyrics(self, track_id: str, track_metadata: dict) -> Lyrics | None:
        """
        Orchestrator function to get the best available synced lyrics.
        It prioritizes syllable-synced lyrics and falls back to line-synced lyrics.
        Returns None if no word-or-better synced lyrics are found.
        """
        lyrics = self.get_syllable_lyrics_only(track_id, track_metadata)
        if lyrics:
            return lyrics

        lyrics = self.get_line_lyrics_only(track_id, track_metadata)
        if lyrics:
            return lyrics

        return None

    def _check_ttml_timing(self, ttml_string: str, allowed_timings: list[str]) -> bool:
        """
        Parses the TTML root element to verify if its timing attribute is in the allowed list.
        e.g., allowed_timings=['Syllable', 'Word']
        """
        try:
            root = ElementTree.fromstring(ttml_string)
            # The namespace is required to find the attribute correctly
            timing_attr = root.get('{http://music.apple.com/lyric-ttml-internal}timing')
            return timing_attr in allowed_timings
        except ElementTree.ParseError:
            return False

    def get_syllable_lyrics_only(self, track_id: str, track_metadata: dict) -> Lyrics | None:
        """
        Downloads and parses ONLY syllable or word-synced lyrics.
        It explicitly checks the TTML content to ensure it's not line-synced.
        """
        ...
        if syllable_ttml:
            # --- CRUCIAL CHECK ---
            # Verify that the fetched TTML is actually syllable or word-timed.
            if self._check_ttml_timing(syllable_ttml, allowed_timings=['Syllable', 'Word']):
                return self._parse_lyrics_ttml(syllable_ttml, is_syllable_timed=True)
        return None

    def get_line_lyrics_only(self, track_id: str, track_metadata: dict) -> Lyrics | None:
        """
        Downloads and parses ONLY line-synced (standard) lyrics.
        It explicitly checks the TTML content to ensure it is line-timed.
        """
        if not track_metadata["attributes"].get("hasLyrics"):
            return None

        lyrics_relationship = track_metadata.get("relationships", {}).get("lyrics", {})
        if lyrics_relationship.get("data"):
            line_synced_ttml = lyrics_relationship["data"][0]["attributes"]["ttml"]
            # --- CRUCIAL CHECK ---
            # Verify that the fetched TTML is actually line-timed.
            if self._check_ttml_timing(line_synced_ttml, allowed_timings=['Line', 'Word']):
                 # We consider "Word" timing acceptable for line-level tasks as well.
    ...
    def _parse_ttml_time(self, time_str: str) -> float:
        """
        Robustly parses various TTML time string formats.
    ...
    def _parse_lyrics_ttml(self, lyrics_ttml: str, is_syllable_timed: bool) -> Lyrics:
        ...
        if self.synced_lyrics_format == SyncedLyricsFormat.TTML:
            lyrics.synced = minidom.parseString(lyrics_ttml).toprettyxml(indent="  ")
            return lyrics
